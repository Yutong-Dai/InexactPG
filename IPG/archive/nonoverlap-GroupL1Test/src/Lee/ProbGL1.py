...
class ProbGL1(Problem):
    ...
    def _ipg_subgradient(self, xk, gradfxk, alphak, x_init, t, epsilon_safeguard, maxiter, scalesubgrad, threshold):
        ...
        rxk = self.r.evaluate_function_value_jit(xk)
        while True:
            ...
            # calculate Q_d(y) = -\frac{\alpha_k}{2}||y-g_k||^2 - r(x_k) - y^Txk
            Qdy = (-alphak / 2) * utils.l2_norm(y - gradfxk)**2 - rxk - np.dot(y.T, xk)
            print("Primal-Dual Gap check:")
            # print(f"mine_unscaled:{myepsilon/self.ck:3.4e} | Lee_unscaled:{-Qdy[0][0]:3.4e} | mine>Lee:{myepsilon/self.ck>-Qdy[0][0]}")
            eta = 0.2
            print(f"mine:{myepsilon:3.4e} | Lee_unscaled:{-Qdy[0][0]*eta:3.4e} | mine>Lee:{myepsilon>-Qdy[0][0]*eta}")

...
def runall(date, solver, loss, lambda_shrinkage, percent, datasets, params, dbDir='../../../db'):
    ...
    if solver == 'naive':
        if params['init_perturb'] != 0.0:
            directory = f"../log/{date}/{solver}/{loss}/{lambda_shrinkage}_{percent}_{params['safeguard_opt']}_{params['safeguard_const']}"
        else:
            directory = f"../log/{date}/{solver}/{loss}/{lambda_shrinkage}_{percent}_exact_{''}"
    elif solver == 'schimdt':

...
class ProbNatOG(Problem):
    ...
    def _xFromY(self, Y, uk, nonZeroGroupFlag):
        ...
        for i in range(self.K):
            if nonZeroGroupFlag[i]:
                ...
                Ystart, Yend = self.r.Ystarts[i], self.r.Yends[i]
                x[start:end] -= Y[Ystart:Yend]
                gradY[Ystart:Yend] = -x[start:end] + 0.0
        x[x < 0] = 0.0
        return x, gradY
    ...
    #     return Y
    def _proj(self, Y, x, stepsize, alphak, nonZeroGroupFlag, gradY):
        """
            perform projection of Y such that
            ||Yg|| <= alphak * wg
        """
        ...
        for i in range(self.K):
            if nonZeroGroupFlag[i]:
                ...
                Yg = Y[Ystart:Yend] + x[xstart:xend] * stepsize
                gradYg = gradY[Ystart:Yend]
                for j in range(len(Yg)):
                    print(f" {Y[Ystart:Yend][j][0]:3.3e} | {(x[xstart:xend][j] * stepsize)[0]:3.3e} | {Yg[j][0]:3.3e} | Y-stepsize*Ygrad:{(Y[Ystart:Yend][j]-stepsize*gradYg[j])[0]:3.3e} | {x[xstart:xend][j][0]:3.3e} | -Ygrad:{-gradYg[j][0]:3.3e}|")
                norm_Yg = utils.l2_norm(Yg)
                radiusg = alphak * self.Lambda_group[i]
                if norm_Yg > radiusg:
    ...
    def _projectedGD(self, xk, gradfxk, alphak, params, outter_iter, Y_init=None, outID=None):
        ...
        while True:
            ...
            while True:
                ...
                Ytrial = self._proj(Y, x, params['projectedGD']['stepsize'], alphak, nonZeroGroupFlag, gradY)
                print("\n Ynew is:")
                for i in range(len(Ytrial)):
                    print(f" {Ytrial[i][0]:3.3e}", end="")
                    if (i + 1) % 5 == 0:
                        print("")
                xtrial, gradYtrial = self._xFromY(Ytrial, uk, nonZeroGroupFlag)
                print("\n xnew is:")

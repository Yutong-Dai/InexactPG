...
class ProbGL1(Problem):
    ...
    def _ipg_subgradient(self, xk, gradfxk, alphak, x_init, t, epsilon_safeguard, maxiter):
        ...
        primalMin = self._prox_primal(x, xk, gradfxk, alphak)
        primalold = None
        # fbest = np.inf
        # # ref
        # # gap reference
        ...
        while True:
            ...
            if iters == 0:
                ...
                if self.primal <= primalold:
                    deltak = deltak * 1.01
                else:
                    deltak = max(0.8 * deltak, 1e-4)
            if self.overflow:
            ...
            iters += 1
            primalOptEst = primalMin - deltak
            stepsize = (self.primal - primalOptEst) / norm_subgrad**2
            x = x - stepsize * subgrad
            primalold = self.primal + 0.0

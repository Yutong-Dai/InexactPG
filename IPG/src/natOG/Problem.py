        ...
        while True:
            ...
            while True:
                ...
                if bak > 50:
                    ...
                    return self._correct_sign(ztrial, entrySignFlag), Y, flag, iters, -999, -999,-999
                bak += 1
                stepsize *= params['xi']
            norm_d = utils.l2_norm(d) * stepsize
            # ----------------------- check termination -------------------------------------
            # if bak == 0:
            #     params['projectedGD']['stepsize'] = min(1e100, params['projectedGD']['stepsize']*1.1)
            # else:
            #     params['projectedGD']['stepsize'] = max(1e-100, params['projectedGD']['stepsize']*0.9)
            ...
            if params['inexact_type'] == 1:
                ...
                epsilon = self.ck * (np.dot(sk.T, sk)[0][0]) * alphak
                aa = 0.5
                bb = utils.l2_norm(xkplus1 - uk) + alphak * self.M 
            elif params['inexact_type'] == 2:
                sk = xkplus1 - xk
                epsilon = (1 - params['nu']) * (1 - params['gamma2']) * (np.dot(sk.T, sk)[0][0]) / 2
                bb = utils.l2_norm(xkplus1 - uk) + alphak * self.M 
                aa = (1 - (1 - 1 / params['nu']) * (1 - params['gamma2'])) / 2
            ...
            if inexact_cond:
                flag = "desired_"
                break
            # print(f"outter_iter:{outter_iter:5d} | inner_iter:{iters:5d} | gap_change:{np.abs(gap - dual_gap_last_iter):3.5e}")
            if np.abs(gap - dual_gap_last_iter) <= np.abs(gap -theta_s) * 0.01 or norm_d <= 1e-12:
                flag = "numetol_"
                break
            dual_gap_last_iter = gap

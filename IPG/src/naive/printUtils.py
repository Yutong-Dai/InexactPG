...
Last Modified: 2021-03-23 17:14
--------------------------------------------
Description:
'''
...
def print_algorithm(algodic, outID=None):
    ...
    with open(filename, "a") as logfile:
        ...
        for k, v in algodic.items():
            if k not in ['prob', 'printlevel', 'printevery', 'max_attempts', 'version', 't']:
                count += 1
                contents += f" {k}: {v} "
                if count % 4 == 3:
                    contents += '\n'
...
def print_header(outID=None):
    if outID is not None:
        ...
        filename = 'log.txt'
    column_titles = '  Iter      F    |   alpha     t      c     its    gap     epsilon  |prox-aprox|  prox-optim/aprox-optim  p-nnz/ap-nnz  p-nz/ap-nz |  bak   stepsize |\n'
    with open(filename, "a") as logfile:

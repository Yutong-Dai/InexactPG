/**
 * File: /Users/ym/Dropbox (LU Student)/ipg/test/slep.cpp
 * Project: /Users/ym/Dropbox (LU Student)/ipg/test
 * Created Date: 2021-09-06 2:50
 * Author: Yutong Dai yutongdai95@gmail.com
 * -----
 * Last Modified: 2021-09-06 5:42
...
void identifySomeZeroEntries(double *u, int *zeroGroupFlag, int *entrySignFlag, int *pp, int *gg, double *v,
                             double lambda1, double lambda2, int p, int g, double *w, double *G)
{
    ...
    for (i = 0; i < g; i++) zeroGroupFlag[i] = 1;
            ...
            if (zeroGroupFlag[i])
            {
                ...
                */
                ...
                if (twoNorm <= lambda2 * w[3 * i + 2])
                {
                    ...
                    for (j = (int)w[3 * i]; j <= (int)w[3 * i + 1]; j++) u[(int)G[j]] = 0;
...
void YFromx(double *Y, double *xnew, double *Ynew, double lambda2, int g, int *zeroGroupFlag, double *G, double *w)
{
    ...
    for (i = 0; i < g; i++)
    {
        ...
        { /*this group is non-zero*/
            ...
            if (twoNorm > 0)
            { /*if x_{G_i} is non-zero*/
                temp = lambda2 * w[3 * i + 2] / twoNorm;

                for (j = (int)w[3 * i]; j <= (int)w[3 * i + 1]; j++) Y[j] *= temp;
            }
            else /*if x_{G_j} =0, we let Y^i=Ynew^i*/
            {
                for (j = (int)w[3 * i]; j <= (int)w[3 * i + 1]; j++) Y[j] = Ynew[j];
            }
        }
    } /*end of for(i=0;i<g;i++) */
}
...
void dualityGap(double *gap, double *penalty2, double *x, double *Y, int g, int *zeroGroupFlag, double *G, double *w,
                double lambda2)
{
    int    i, j;
    ...
    for (i = 0; i < g; i++)
    {
        ...
        { /*this group is non-zero*/
            ...
            *penalty2 += twoNorm;
            ...
            if (twoNorm > innerProduct) *gap += twoNorm - innerProduct;
        }
    } /*end of for(i=0;i<g;i++) */
}
...
void overlapping_gd(double *x, double *gap, double *penalty2, double *v, int p, int g, double lambda1, double lambda2,
                    double *w, double *G, double *Y, int maxIter, int flag, double tol)
{
    ...
    identifySomeZeroEntries(u, zeroGroupFlag, entrySignFlag, &pp, &gg, v, lambda1, lambda2, p, g, w, G);
    ...
    for (i = 0; i < g; i++)
    {
        ...
        { /*this group is zero*/
            for (j = (int)w[3 * i]; j <= (int)w[3 * i + 1]; j++) Y[j] = 0;
        }
    }
    ...
    for (iterStep = 0; iterStep < maxIter; iterStep++)
    {
            ...
            for (i = 0; i < g; i++)
            {
                ...
                { /*this group is non-zero*/
                    ...
                    if (twoNorm > lambda2 * w[3 * i + 2])
                    {
                        ...
                        for (j = (int)w[3 * i]; j <= (int)w[3 * i + 1]; j++) Ynew[j] *= temp;
                    }
                }
            } /*end of for(i=0;i<g;i++) */
            ...
            else
            {
                ...
                if (L / g - 2 * g)
                {
                    ...
                    else
                    {
                        ...
                        printf(
                            "\n L=%e > 2 * %d * %d. There might be a bug here. Otherwise, it is due to numerical "
                            "issue.",
                            L, g, g);
        ...
        else
        {
            ...
            { /*we use (x, Y), as it is better in terms of duality gap*/
                *gap = gapR;
                *penalty2 = penalty2R;
            }
    ...
    for (i = 0; i < g; i++)
    {
        ...
        else
        {
            for (j = (int)w[3 * i]; j <= (int)w[3 * i + 1]; j++)
            {
                if (x[(int)G[j]] != 0) break;
            }

            if (j > (int)w[3 * i + 1]) penalty2[4] = penalty2[4] + 1;
        }
    }
...
void gradientDescentStep(double *xnew, double *Ynew, double *LL, double *u, double *y, int *entrySignFlag,
                         double lambda2, double *x, double *Y, int p, int g, int *zeroGroupFlag, double *G, double *w)
{
        ...
        else
        {
            ...
            if (L / g - 2 * g > 0)
            {
                ...
                else
                {
                    ...
                    printf("\n L=%e > 2 * %d * %d. There might be a bug here. Otherwise, it is due to numerical issue.",
                           L, g, g);
...
void overlapping_agd(double *x, double *gap, double *penalty2, double *v, int p, int g, double lambda1, double lambda2,
                     double *w, double *G, double *Y, int maxIter, int flag, double tol)
{
    int     YSize = (int)w[3 * (g - 1) + 1] + 1;
    double *u = (double *)malloc(sizeof(double) * p);
     ...
     *
     */
    for (i = 0; i < YSize; i++) YS[i] = Yp[i] = Ynew[i] = 0;
    ...
    gradientDescentStep(xnew, Ynew, &L, u, y, entrySignFlag, lambda2, x, Y, p, g, zeroGroupFlag, G, w);
            ...
            else
            {
                ...
                *penalty2 = penalty2R;
                ...
                else
                {
                     ...
                     * double *G, double *w)
                     */
                    gradientDescentStep(x, Y, &L, u, y, entrySignFlag, lambda2, xS, YS, p, g, zeroGroupFlag, G, w);
...
void overlapping(double *x, double *gap, double *penalty2, double *v, int p, int g, double lambda1, double lambda2,
                 double *w, double *G, double *Y, int maxIter, int flag, double tol)
{
    // for (int i = 0; i <= 8; i++) printf("w[%d]:%f\n", i, w[i]);
    // for (int i = 0; i < p; i++)
    // {
    //     printf("v[%d]:%f\n", i, v[i]);
    // }
    ...
    {
        case 1:
            overlapping_gd(x, gap, penalty2, v, p, g, lambda1, lambda2, w, G, Y, maxIter, flag, tol);
            break;
        case 2:
            overlapping_agd(x, gap, penalty2, v, p, g, lambda1, lambda2, w, G, Y, maxIter, flag - 2, tol);

            break;
        default:
            /* printf("\n Wrong flag! The value of flag should be 0,1,2,3. The program uses flag=2.");*/

            overlapping_agd(x, gap, penalty2, v, p, g, lambda1, lambda2, w, G, Y, maxIter, 0, tol);
            break;
    }
}

/**
 * @brief compute the overlapping group l1 regularizer of the form
 *        min_{x} 0.5 * ||x-v||^2 + lambda1 * ||x||_1 + lambda2 * \sum_{g\in G} w_g||[x]_g||_2
 * where G is a collection of groups (may overlap) include all variables
 * \param v: the point to calculate
 * \param p: number of variables
 * \param ng: number of groups
 * \param lambda1
 * \param lambda2
 * \param w: an array of length 3*g; w[3*i] the starting index of the i-th group;
 *                                   w[3*i+1] the ending index of the i-th group;
 *                                   w[3*i + 2] the weight w_g for the i-th group;
 *
 * \param G:* an array containing the indices for the groups G_g1, G_g2, ..., G_ng
 * \param Y: an array of length len(G) to store the dual variable value
 * \param maxiter: max number of iterations allow to perform dual graident ascent
 * \param flag: 2 use AGD; 1 use GD
 * \param tol: desired tolerance to terminate the algorithm
 * \param solution: the solution with the desired accuracy
 * \param gap:  the calculated duality gap at the solution
 * \param info: info[0]: the value of group regularizer evaluated at the solution; info[1]: number of nonzero elements
 *              info[2]: number of non-zero groups; info[3]: number of iterations; info[4]: number of zero groups
 */
void overlapping_py(py::array_t<double> v, int p, int ng, double lambda1, double lambda2, py::array_t<double> w,
                    py::array_t<double> G, py::array_t<double> Y, int maxiter, int flag, float tol,
                    py::array_t<double> solution, double gap, py::array_t<double> info)
{
    auto vptr = static_cast<double *>(v.request().ptr);
    auto wptr = static_cast<double *>(w.request().ptr);
    auto Gptr = static_cast<double *>(G.request().ptr);
    auto Yptr = static_cast<double *>(Y.request().ptr);
    auto solutionptr = static_cast<double *>(solution.request().ptr);
    auto infoptr = static_cast<double *>(info.request().ptr);
    overlapping(solutionptr, &gap, infoptr, vptr, p, ng, lambda1, lambda2, wptr, Gptr, Yptr, maxiter, flag, tol);
}

PYBIND11_MODULE(slep, m)
{
    m.doc() = "A python wrapper for Overlapping.h from slep package.";
    m.def("overlapping_py", &overlapping_py, "compute proximal operator with the overlapping group l1 regularizer ");
}
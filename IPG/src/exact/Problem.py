...
class ProbOGL1(Problem):
    ...
    def ipg(self, xk, gradfxk, alphak, params, lambda_init=None, rxk=None, outter_iter=None):
        ...
        while True:
            ...
            gap, primal, LB = self._duality_gap(x, y, uk, alphak, params['rtype'])
            if params['inexact_type'] == 1:
                ...
                phi_p_xk = utils.l2_norm(xk - uk) ** 2 / (2 * alphak) + rxk
                epsilon = params['gamma2'] * (phi_p_xk - LB)
                condition1 = gap <= epsilon
                condition2 = epsilon <= (phi_p_xk - primal)
                inexact_cond = condition1 & condition2
    ...
    def _duality_gap(self, x, y, uk, alphak, rtype):
        temp = x - uk
        if rtype == 'exact':
        ...
        gap = primal + dual_negative
        LB = -dual_negative
        return gap, primal, LB

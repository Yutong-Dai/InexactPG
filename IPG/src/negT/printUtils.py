...
def print_header(outID=None):
    if outID is not None:
        ...
        filename = 'log.txt'
    column_titles = '  Iter      F    |   alpha        t        c     its    gap    epsilon  type |prox-aprox|  prox-optim/aprox-optim  p-#nz/ap-#nz  p-#z/ap-#z |  bak   stepsize  |d_full| |\n'
    with open(filename, "a") as logfile:
...
def print_proximal_update(alpha, t, c, subits, gap, epsilon, eflag,
    ...
    if outID is not None:
        ...
        filename = 'log.txt'
    contents = f" {alpha:2.3e}  {t:+2.2e} {c:2.2e} {subits:3d} {gap:2.3e} {epsilon:2.3e}  {eflag}   {prox_diff:2.3e}    {prox_optim:2.3e}/{aprox_optim:2.3e}      {prox_nnz:4d}/{aprox_nnz:4d}    {prox_nz:4d}/{aprox_nz:4d}  |"
    with open(filename, "a") as logfile:
        logfile.write(contents)
...
def print_proximal_update_failed(alpha, t, c, subits, gap, epsilon, outID=None):
    if outID is not None:
        ...
        filename = 'log.txt'
    contents = f" {alpha:2.3e}  {t:+2.2e} {c:2.2e} {subits:3d} {gap:2.3e} {epsilon:2.3e}  \n"
    with open(filename, "a") as logfile:
        logfile.write(contents)

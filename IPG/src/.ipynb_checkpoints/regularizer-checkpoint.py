...
class OGL1:
    ...
    def func_ub(self, X):
        return _fub_jit(X, self.K, self.starts, self.ends, self.Lambda_group, self.freq)
...
@jit(nopython=True)
def _fub_jit(X, K, starts, ends, Lambda_group, freq):
    ub = 0.0
    for i in range(K):
...
def _dual_jit(y, K, starts, ends, Lambda_group):
    ...
    for i in range(K):
        start, end = starts[i], ends[i]
        yg = y[start:end]
        temp = (np.sqrt(np.dot(yg * yg))) / Lambda_group[i]

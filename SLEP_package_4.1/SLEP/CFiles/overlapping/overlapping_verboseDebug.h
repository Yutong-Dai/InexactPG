...
/* p: problem dim; g: num of groups*/
/* pp: num of zero coordinates; gg: num of zero groups*/
void identifySomeZeroEntries(double * u, int * zeroGroupFlag, int *entrySignFlag,
    ...
    /* zeroGroupFlage[i] = 1 means group i is non-zero */
    for(i=0;i<g;i++)
        zeroGroupFlag[i]=1;
    ...
    for(i=0;i<g;i++){
        if(zeroGroupFlag[i]){ /*this group is non-zero*/
            ...
            *penalty2+=twoNorm;
            ...
            printf("\n group %d | twoNorm:%3.3e | innerProduct:%3.3e", i, twoNorm, innerProduct);
...
void overlapping_gd(double *x, double *gap, double *penalty2,
    ...
    printf("Call identifySomeZeroEntries():\n");
    printf("zeroGroupFlag:\n");
    for(i=0;i<g;i++){
         printf("%d ",zeroGroupFlag[i]);
         if ((i+1) % 5 == 0){printf("\n");}
    }
    printf("\nentrySignFlag:\n");
    for(i=0;i<p;i++){
         printf("%d ",entrySignFlag[i]);
         if ((i+1) % 5 == 0){printf("\n");}
    }
    printf("\nwork on u:\n");
    for(i=0;i<p;i++){
    ...
    for(iterStep=0;iterStep<maxIter;iterStep++){
         ...
         */
        printf("=========================================\n");
        printf("\niter: %d | PGD stepsuze: %3.3e", iterStep, 1/L);
        printf("\n Current Y is:\n");
        for(i=0;i<YSize;i++){
             printf(" %3.3e",Y[i]);
             if ((i+1) % 5 == 0){printf("\n");}
...
//             }
            ...
            printf("\n Test sufficient decrease: lhs: %3.3e | rhs: %3.3e", leftValue, rightValue * L);
            ...
            if ( leftValue <= L * rightValue){
                ...
                if (L*2 <= temp){
                    L=temp;
                    printf("\n L is set to: left/right=%3.3e", temp);
                }
                    
                else{
                     L=2*L;
                     printf("\n L is set to: 2L=%3.3e", L);
                }
                   
                
                if ( L / g - 2* g ){
                    printf("\n terminate without meet suffcient decreases");
                    if (rightValue < 1e-16){
                        break;
                    }
                    else{
                        ...
                        printf("\n L=%e > 2 * %d * %d. There might be a bug here.\n Otherwise, it is due to numerical issue.", L, g, g);
                        printf("\n ====End of BAK ERROR====");
                        break;
                    }
                }                
        ...
        normd =sqrt(normd);
        printf("\n bak:%d | stepsize for next iter:%f | norm_d:%3.3e", bak, 1/L, normd);

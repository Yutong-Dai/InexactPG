...
def create_paths(logdir='../IPG/test/log', date='04_18_2021', inexact_type=1, loss='logit',
                 subsolver='projectedGD', warm_start=True,
                 lam_shrink=[0.8], group_size=[10], overlap_ratio=[0.1], excludes=None, *argv):
    list_all_npy_path = []
    for lam in lam_shrink:
        for grp in group_size:
            for r in overlap_ratio:
                minimal_dir = f'{logdir}/{date}/{inexact_type}/{loss}/{subsolver}_{warm_start}_{lam}_{grp}_{r}'
                for arg in argv:
                    minimal_dir += f'_{arg}'
...
def get_all(logdir, date, inexact_type, loss, subsolver, warm_start,
            lam_shrink, group_size, overlap_ratio,
            excludes=None, param1_lst=None, param2_lst=None):
    algo_df_dict = {}

    for p1 in param1_lst:
        for p2 in param2_lst:
            algorithm = f'{inexact_type}-{subsolver}-{warm_start}-{p1}-{p2}'
            print(f'{algorithm}')
            paths = create_paths(logdir, date, inexact_type, loss,
                                 subsolver, warm_start,
                                 lam_shrink, group_size, overlap_ratio, excludes, p1, p2)
            if paths == []:
                print(' empty')
                df = None
            else:
...
def _get_all(logdir, date, inexact_type, loss, subsolver, warm_start,
            lam_shrink, group_size, overlap_ratio,
            excludes=None, param1_lst=None, param2_lst=None):
    lst = []
    for p1 in param1_lst:
